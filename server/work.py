...
def get_job():
    ...
    query_0 = Jobs.query.filter(Jobs.status==0).filter(or_(Jobs.tag == '', Jobs.tag.in_(tag.strip().replace(' ', '').split(','))))
...
def get_job_summary():
    mysql_0 = "select a.job_type, a.batch, a.tag, sum(case when a.status = 0 then 1 else 0 end) as pending_task, sum(case when a.status = 1 then 1 else 0 end) as working_task, sum(case when a.status = 3 then 1 else 0 end) as finished_task, sum(case when a.status = -1 then 1 else 0 end) as failed_task, min(a.create_time) as begin_time, max(a.update_time) as update_time from jobs a group by a.job_type, a.batch"
    res = db.session.execute(mysql_0)
    data = convert_rowproxy_to_dict(res.fetchall())
    return jsonify(code=200, data=data)
